# Classe base per tutti i metodi di pagamento
class MetodoPagamento:
    
    def __init__(self, nome, numero_carta, data_scadenza, cvc):  #iniziallizazione dei parametri privati
        # attributi privati della classe genitore MetodoDiPagamento (incapsulamento)
        self.__nome = nome
        self.__numero_carta = numero_carta
        self.__data_scadenza = data_scadenza
        self.__cvc = int(cvc)

    def metodo_pagamento(self):    #inizializzaione generica pdel metodo di pagamento
        print("Metodo di pagamento generico")


class CartaDiCredito(MetodoPagamento):  #classe figlia (ereditarietà)

    classe = "CartaDiCredito"   #nome classe associata al dizionario

    def __init__(self, nome, numero_carta, data_scadenza, cvc, saldo=1000):  #attributi 
        super().__init__(nome, numero_carta, data_scadenza, cvc)
        self.__saldo = saldo    # Saldo disponibile sulla carta

    def metodo_pagamento(self):
        # Implementazione del metodo di pagamento per la carta di credito.
...
# Classe per gestire il pagamento tramite PayPal
class PayPal(MetodoPagamento):

    classe = "PayPal"   # Tipo di pagamento associato
