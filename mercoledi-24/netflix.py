...
class Netflix:
    ...
    def scrivi_df(self):
        self.df.to_csv("mercoledi-24/netflix_titles (1).csv")
    ...
    def grafico_distribuzioni_paesi(self):
        self.pulizia()
        df_country = self.df[["type","country"]].dropna().copy()
        df_country.loc["country"] = df_country["country"].apply(lambda x: x.split(',')[0])
        df_country_group = df_country.groupby("country").size().sort_values(ascending=False).head(10)
        plt.figure(figsize=(10, 6))
        df_country_group.plot(kind='bar', color='orange')
        plt.title('Distribuzione dei prodotti per paese su Netflix (Top 10)')
        plt.xlabel('Paese')
...
def main():
    ...
    a = Netflix("mercoledi-24/netflix_titles (1).csv")
    while True:
        s = menu()
        if s == "1":
            a.visualizza_df()
            ...
            a.grafico_film_serie_a_torta()
        elif s == "7":
            a.grafico_andamento_tempo_prodotti()
        elif s == "0":
            a.grafico_distribuzioni_paesi()
            break
        else:
            print("\nScelta non valida!")
